...
<body>
    ...
    <header class="bg-white">
            ...
            <div class="flex h-16 items-center justify-between">
                ...
                <div class="flex items-center gap-4">
                    <div class="flex sm:gap-4">
                        <!-- Show only on smaller screens (sm and below) -->
                        @if (Route::has('login'))
                            @auth
                            ...
                            @else
                                <a class="rounded-md bg-gray-900 px-5 py-2.5 text-sm font-medium text-white shadow-sm sm:hidden"
                                    href="/login">
                                    Login
                                </a>

                                <!-- Show both Login and Register on medium screens (md and above) -->
        ...
        <div id="mobile-menu"
            class="md:hidden hidden bg-white shadow-lg absolute w-full top-16 left-0 z-10 max-h-[70vh] overflow-y-auto">
            <!-- Close Button -->
            ...
            <nav>
                <ul class="flex flex-col gap-6 text-lg py-4 px-6">
                    <li>
                        <a class="text-gray-900 transition hover:text-gray-500/75" href="#"> About </a>
                    </li>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const periods = document.querySelectorAll('[data-monthly]');

            function togglePricing(isAnnual) {
                prices.forEach(price => {
                    const monthlyPrice = price.getAttribute('data-monthly');
                    const annualPrice = price.getAttribute('data-annually');
                    price.textContent = isAnnual ? annualPrice : monthlyPrice;
                });

                periods.forEach(period => {
                    const monthlyPeriod = period.getAttribute('data-monthly');
                    const annualPeriod = period.getAttribute('data-annually');
                    period.textContent = isAnnual ? annualPeriod : monthlyPeriod;
                });

                monthlyBtn.classList.toggle('bg-gray-800', !isAnnual);
                monthlyBtn.classList.toggle('text-white', !isAnnual);
                annuallyBtn.classList.toggle('bg-gray-800', isAnnual);
                annuallyBtn.classList.toggle('text-white', isAnnual);
            }

            monthlyBtn.addEventListener('click', () => togglePricing(false));
            annuallyBtn.addEventListener('click', () => togglePricing(true));
        });
    </script>
    <script>
